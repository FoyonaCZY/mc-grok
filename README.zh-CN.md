...
**100% 由 Grok 4.6 编写的纯前端《Minecraft》复刻。**（Minecrafts 1.22）
...
**战斗：** 弓、箭、盾牌（按住右键格挡）、不死图腾（主手或副手）。工具和盔甲有耐久条。
...
- 铁傀儡建造、狼驯养、栅栏门开关
- 指南针 HUD、时钟 HUD、F3 调试（F3+C 复制传送坐标）、中英界面
- 进度系统（暂停菜单 → 进度）与弹出提示
- 攻击冷却、冲刺视野、水/岩浆/传送门/着火画面
- 金苹果：再生 + 伤害吸收；金胡萝卜：夜视；女巫近战可能中毒
- 约 40 秒自动保存；选项里可开「死亡不掉落」
- 亮度滑条；「流畅」图形会关掉环境光遮蔽和额外光照扩散以换帧率
- 昼夜、雨雪、程序生成音效（不是原版采样）
...
/locate village
/locate stronghold
/effect regen|speed|fire_resist|poison|night_vision|absorption [秒]
/help
```
...
- 区块：`Uint8Array` 方块 ID（不超过 255）+ 光照扩散（流畅画质会简化）
- 区块流式生成，网格按距离优先重建
